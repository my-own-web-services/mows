...
# firstdorsal/pixiecore
# quay.io/pixieboot/pixieboot
docker run \
  --rm --name pixiecore --net=host -v $PWD:/files firstdorsal/pixiecore \
    boot /files/${BASENAME}-kernel /files/${BASENAME}-initrd -d --cmdline="rd.neednet=1 rd.live.overlay.overlayfs=1 ip=dhcp rd.cos.disable root=live:{{ ID \"/files/${BASENAME}.squashfs\" }} netboot nodepair.enable config_url={{ ID \"/files/config-${1}.yaml\" }} console=tty1 console=ttyS0 console=tty0"
