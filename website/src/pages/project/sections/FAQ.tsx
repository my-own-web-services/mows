...
const faq: FAQItem[] = [
    ...
    },
    {
        ...
        answer: "For the same price as a single server, you can have three servers that are more reliable and can be easier maintained. One server fails? Just rip it out and replace it with a new one. No need to worry about data loss or downtime. The hardware part of the project aims to deliver a affordable computer cluster option that is easy to setup and not just a tech enthusiast experimenting toy but an actual reliable solution. As MOWS can easily run virtual machines you can easily stream any desktop operating system to a super cheap thin client, cutting out the need for many expensive desktops. This way you can make the most out of your hardware and have a reliable system that can grow with you. A cluster like this could also be easily shared with friends or family, making it even more cost effective. You can also cut out expensive cloud services and the cost of expensive mobile devices that you bought to have that easy to use but walled off ecosystem. The MOWS services can run on any device that can run a web browser, so you can have the ease of use on any cheaper device."
    },
    {
    ...
    {
        question: "Hardware: Why not use a Raspberry Pi?",
        answer: "Raspberry Pi is a great platform for learning and experimenting with technology, but it is not a great platform for running a production system. The performance for that ever increasing price as well as its limits in terms of memory and CPU instructions (Virtualisation, Video encoding, etc.) make it less and less interesting to use compared to the other options out there."
    },
    {
    ...
    },
    {
        ...
        answer: "No, you can run the services on your local network and access them from any device that is connected to the same network. When outside you need to have a internet connection to access the services. The MOWS Cloud APIs make it easy to create well working offline first web applications that can sync with the server when a connection is available."
    },
    {
    ...
    },
    {
        ...
        answer: "The idea of decentralization is great, but it is absurd to believe that a complicated ecosystem with so much legacy code could be replaced solely by some version of a blockchain. However, Blockchain and similar technologies may very well be part of the future and the solution to some problems, that we want to solve with MOWS. Some form of decentralized payment system will be required for a true sovereign private and independent world wide web. This will be included into the API stack at some point."
    }
];
            ...
            >
                ...
                <div>
                    {faq.map(({ question, answer }) => (
                        <Collapsible
                            title={<span className={"font-bold text-xl"}>{question}</span>}
                            key={question}
                        >
                            <p>{answer}</p>
                        </Collapsible>
