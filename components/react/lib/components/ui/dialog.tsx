    ...
    return (
        <DialogPortal data-slot={`dialog-portal`}>
            ...
            <DialogPrimitive.Content
                ...
                className={cn(
                    `bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 flex max-h-[90vh] w-full max-w-[90vw] translate-x-[-50%] translate-y-[-50%] flex-col gap-4 rounded-lg border shadow-lg duration-200`,
                    className
                )}
                {...props}
            >
...
const DialogHeader = ({ className, ...props }: React.ComponentProps<`div`>) => {
    return (
        <div
            ...
            className={cn(`flex flex-col gap-2 pt-6 pl-6 text-center sm:text-left`, className)}
            {...props}
        />
    );
};
