...
>(
  (
    ...
    const clampWidth = React.useCallback(
      (px: number) => Math.max(minWidthPx, Math.min(maxWidthPx, px)),
      [minWidthPx, maxWidthPx]
    )

    const [widthPx, _setWidthPx] = React.useState<number>(() => {
      const stored = readWidthCookie()
      return Math.max(
        minWidthPx,
        Math.min(maxWidthPx, stored ?? defaultWidthPx)
      )
    })
    ...
    // Re-clamp persisted width if min/max bounds change.
    React.useEffect(() => {
      _setWidthPx((current) => clampWidth(current))
    }, [clampWidth])
      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            style={
              {
                "--sidebar-width": resizable ? `${widthPx}px` : SIDEBAR_WIDTH,
        ...
        >
          ...
          {resizable && (
            <div
              ...
              className={cn(
                `absolute inset-y-0 z-20 w-1 cursor-col-resize bg-transparent transition-colors hover:bg-sidebar-border active:bg-sidebar-border`,
