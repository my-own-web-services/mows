...
//!
//! Compiles the same `OpenApiRouter` graph the server binary uses
//! (`mows_package_manager::api::openapi::build_api_router`) and writes the
//! resulting OpenAPI document to stdout (or `--output <path>`).
//!
//! No config load, no observability, no network — the router is consumed
//! by `OpenApiRouter::into_openapi(self)`, which only reads the compile-
//! time `utoipa` metadata. Replaces "boot the server, `curl
//! /apidoc/openapi.json`" in `operators/mows-package-manager/scripts/codegen.sh`.

use mows_package_manager::api::openapi::build_api_router;
use std::env;
use std::fs;
