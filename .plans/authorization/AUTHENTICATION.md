...
1. Validates the bearer token via Zitadel introspection.
2. Looks up `mows_auth.users` by `external_user_id = sub`.
...
need: `id`, `external_user_id`, `display_name`, `email`, `user_type`
(normal vs super-admin). Profile pictures, MFA settings, sessions —
all stay in Zitadel.
...
| `external_user_id`      | Zitadel `sub` claim                                 |
| `display_name`          | Zitadel `name` / `preferred_username`               |
| `email`                 | Zitadel `email` (if `email_verified`)               |
| `user_type`             | MOWS-only (`Normal`, `SuperAdmin`); not in Zitadel  |
...
| `external_client_id`    | Zitadel OIDC client_id (the new authoritative field)   |
...
The `external_client_id` is the new join key replacing the
origin-based lookup as the primary path. The origin lookup is
retained as a secondary check for Frontend apps (it's a free
defense-in-depth signal; an attacker who steals a refresh token still
has to come from a registered origin).
...
group), filez has two choices:

- **Forward the user's token** to realtime. Simplest; realtime sees
  the call as pattern 2. Downside: the user's token leaves filez's
  process. Acceptable when the destination is another in-cluster API
  we control.
- **Use filez's own service-account token + an on-behalf-of header**
  (pattern 4 / 5 hybrid). The calling API is filez; the
  `X-Mows-On-Behalf-Of` is the user. Realtime's middleware runs the
  same EXISTS check as it would for any backend impersonator —
  except the impersonator here is another API. The user must have
  authorized filez to impersonate them to realtime, which by the
  cluster's standard install is implicit (cluster-internal APIs are
  pre-trusted to fan out user actions to each other via a seed
  policy).

Recommendation: prefer the second pattern for cleanliness — the
user's token never crosses process boundaries other than from
browser to first API. The seed "APIs may impersonate the user to
other cluster APIs" policy is created at install time by the manager
and is auditable like any other policy row.
...
| New first-party app installed                  | The app's install manifest declares its Zitadel client metadata; mpm registers it with Zitadel and inserts the `mows_auth.apps` row. |
| New third-party SPA registers                  | Developer registers via Zitadel admin UI (or a self-service flow gated by an admin policy). The new client_id is mirrored into `mows_auth.apps` on first use, with `trusted = false`. |
...
| Two Zitadel clients accidentally registered with the same `external_client_id` | Should be impossible — `external_client_id` has a UNIQUE constraint in `mows_auth.apps`. Cluster install scripts must use Zitadel's API to discover existing clients before creating new ones. |

## 10. Decision summary

| Question                                                                  | Decision                                                                       |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Identity provider for v1                                                  | Zitadel                                                                        |
| Are user identities stored in MOWS too?                                   | Yes — thin `mows_auth.users` row, JIT-provisioned, keyed by Zitadel `sub`      |
...
- **Multiple identity providers in parallel.** v1 is Zitadel-only.
  Adding a second IdP would need a discriminator column in
  `mows_auth.users.external_user_id`. Not on the v1 roadmap.
