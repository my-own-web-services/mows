...
interface ColumnRowComponentState {}

class ColumnRowComponent<FilezResourceType extends BaseResource> extends PureComponent<
    RowComponentProps<FilezResourceType>,
    ColumnRowComponentState
> {
    constructor(props: RowComponentProps<FilezResourceType>) {
        super(props);

        this.state = {};
    }

    getCurrentItem = (): FilezResourceType => {
        return this.props.data?.items?.[this.props.index]!;
    };

    onItemClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        this.props.data?.handlers.onItemClick?.(e, this.getCurrentItem(), this.props.index);
    };
    ...
    onContextMenu = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        this.props.data?.handlers.onItemClick?.(e, this.getCurrentItem(), this.props.index, true);
    };

    canDrop = () => {
        //return (
        //    this.props.data?.rowHandlers?.isDroppable?.(this.getCurentItem() as ResourceType) ??
        //    false
        //);
    };
    ...
    render = () => {
        ...
        const isSelected = this.props.data.selectedItems[this.props.index] === true;
        const style = this.props.style;
        const itemType = this.props.data.resourceType;
        const isLastSelected = this.props.data.lastSelectedItemIndex === this.props.index;
        ...
        return (
            <div
                ...
                className={cn(
                    `ColumnListRowRenderer select-none`,
...
const ColumnListRowHandler = <
    FilezResourceType extends BaseResource
>(): ListRowHandler<FilezResourceType> => ({
    name: "ColumnListRowHandler",
    icon: <FaThList style={{ transform: "scale(0.9)", pointerEvents: "none" }} size={17} />,
    component: ColumnRowComponent,
...
export default ColumnListRowHandler;
