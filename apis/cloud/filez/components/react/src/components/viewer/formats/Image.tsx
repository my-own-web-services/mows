...
export default class Image extends PureComponent<ImageProps, ImageState> {
    ...
    render = () => {
        ...
        if (!processedImage && this.props.disableFallback === true) return;
...
export const getRotation = (
    ...
    const rotation =
        filezFile.app_data?.metadata?.result?.exifdata?.Orientation;
...
export const getImagePreviewWidth = (
    filezFile: FilezFile,
    viewportItemWidth: number
): [number, boolean] => {
    viewportItemWidth = viewportItemWidth * window.devicePixelRatio;

    const maybeImageAddon = filezFile.app_data.image as ImageAddon | undefined;
    if (maybeImageAddon !== undefined) {
