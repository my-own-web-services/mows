...
impl ApiClient {
    ...
    /// Returns every policy pinning access to one resource, plus the resource's owner. Sibling of realtime-server's endpoint; both emit the same shape so the authz-admin BFF stays translator-free. Caller must own the resource; non-existence and non-ownership are collapsed into one 403.
    #[tracing::instrument(level = "trace")]
    pub async fn by_resource(
        &self,
        request_body: ByResourceRequestBody,
    ) -> Result<ApiResponseByResourceResponseBody, ApiClientError> {
        let full_url = format!("{}/api/access_policies/by_resource", self.base_url);
    ...
    /// Revoke an access policy (soft-delete; preserves audit trail).
    #[tracing::instrument(level = "trace")]
    pub async fn revoke_access_policy(
        ...
        let full_url = format!(
            "{}/api/access_policies/revoke/{access_policy_id}",
            self.base_url
        );
        let full_url = Url::parse(&full_url).unwrap();
    ...
    /// Paginated audit-log timeline. Resource-scoped when (resource_type, resource_id) are supplied (owner-only); self-scoped otherwise (caller's own actions). Sibling of realtime-server's endpoint; both emit the same wire shape so the authz-admin BFF can forward verbatim.
    #[tracing::instrument(level = "trace")]
    pub async fn list_audit_log(
        &self,
        request_body: ListAuditLogRequestBody,
    ) -> Result<ApiResponseListAuditLogResponseBody, ApiClientError> {
        let full_url = format!("{}/api/audit_log/list", self.base_url);
        let full_url = Url::parse(&full_url).unwrap();
