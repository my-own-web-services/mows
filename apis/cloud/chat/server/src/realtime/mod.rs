...
//! a slow subscriber loses events but the durable `channel_messages`
...
use crate::models::channels::{messages::ChannelMessage, ChannelId};

/// Per-channel event broadcast over WebSocket subscriptions.
/// Mirrors the durable `channel_messages` row shape — clients
...
pub struct ChannelBroadcastRegistry {
    inner: Arc<Mutex<HashMap<ChannelId, broadcast::Sender<ChannelEvent>>>>,
}
...
impl ChannelBroadcastRegistry {
    ...
    pub async fn subscribe(&self, channel_id: ChannelId) -> broadcast::Receiver<ChannelEvent> {
    ...
    /// subscribers exist — the durable `channel_messages` write
    /// happened before this call, so missing the broadcast just
    /// means no one was listening.
    pub async fn publish(&self, channel_id: ChannelId, event: ChannelEvent) {
...
mod tests {
    use super::*;
    use crate::models::{channels::messages::ChannelMessageId, users::ChatUserId};

    fn fake_message(channel_id: ChannelId) -> ChannelMessage {
        ChannelMessage {
            id: ChannelMessageId::new(),
            channel_id,
            author_id: ChatUserId::new(),
    ...
    async fn publish_with_no_subscribers_is_noop() {
        ...
        reg.publish(ch, ChannelEvent::Message { message: fake_message(ch) })
            .await;
        // No panic, no error — publish silently no-ops when nobody
        // is listening. The durable row in channel_messages is the
        // source of truth.
    }
    ...
    async fn subscriber_receives_subsequent_publish() {
        ...
        let msg = fake_message(ch);
        let expected_id = msg.id;
        reg.publish(ch, ChannelEvent::Message { message: msg }).await;
        let received = rx.recv().await.expect("event delivered");
        match received {
            ChannelEvent::Message { message } => assert_eq!(message.id, expected_id),
