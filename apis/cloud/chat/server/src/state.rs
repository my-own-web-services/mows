//! Shared axum state for chat-server.
...
impl AppState {
    ...
    pub async fn new(db_url: &str) -> Result<Self, ChatError> {
        let database = Database::new(db_url).await;
        let context_app = bootstrap_chat_app(&database).await?;
        Ok(Self {
            database,
            context_app,
...
async fn bootstrap_chat_app(database: &Database) -> Result<MowsApp, ChatError> {
    let mut connection = database.get_connection().await?;
    let now = get_current_timestamp();
    let app = MowsApp {
        id: MowsAppId(CHAT_APP_ID),
        name: "chat".to_string(),
    ...
    diesel::insert_into(schema::apps::table)
        ...
        .await?;
    schema::apps::table
        .filter(schema::apps::id.eq(MowsAppId(CHAT_APP_ID)))
...
/// Convenience for handlers / tests that need a [`ChatUserId`]
/// from a UUID.
pub fn user_id(uuid: Uuid) -> ChatUserId {
    ChatUserId(uuid)
}
