...
use crate::{config::config, errors::RealtimeError};
...
impl Database {
    ...
    pub async fn get_connection(&self) -> Result<DatabaseConnection, RealtimeError> {
        match &self.pool {
            Some(pool) => pool.get().await.map_err(RealtimeError::from),
            None => Err(RealtimeError::DatabasePoolNotInitialized),
        }
    }
    ...
    pub async fn run_migrations() -> Result<(), RealtimeError> {
