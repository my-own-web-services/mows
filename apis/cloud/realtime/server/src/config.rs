//! Realtime-server runtime configuration.
//!
//! All env-vars are read here and projected into a single
//! [`RealtimeServerConfig`] struct so the rest of the codebase never
...
static CONFIG_LOCK: OnceLock<RwLock<RealtimeServerConfig>> = OnceLock::new();
...
pub fn config() -> &'static RwLock<RealtimeServerConfig> {
...
pub fn from_env() -> anyhow::Result<RealtimeServerConfig> {
    ...
    Ok(RealtimeServerConfig {
        db_url: load_env(
            "postgres://realtime:realtime@realtime-db/realtime",
            "DATABASE_URL",
            true,
            true,
        )?,
