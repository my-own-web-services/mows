//! Channel + per-channel-event models.
...
use crate::{
    ...
    models::users::UserId, schema, utils::get_current_timestamp,
};
...
impl Channel {
    pub fn new(owner_id: UserId, name: impl Into<String>, topic: Option<String>) -> Self {
    ...
    pub async fn insert(self, database: &Database) -> Result<Channel, RealtimeError> {
    ...
    pub async fn get_by_ids(
        database: &Database,
        ids: &[ChannelId],
    ) -> Result<Vec<Channel>, RealtimeError> {
        let mut connection = database.get_connection().await?;
