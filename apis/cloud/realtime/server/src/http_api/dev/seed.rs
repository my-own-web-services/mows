...
//! UUIDs so a client can hand them straight to the X-Realtime-User-Id
...
pub async fn dev_seed(
...
) -> Result<Json<ApiResponse<DevSeedResponse>>, RealtimeError> {
    ...
    for (id, name) in [(ALICE_ID, "Alice"), (BOB_ID, "Bob")] {
        let exists: Option<User> = schema::users::table
            .filter(schema::users::id.eq(UserId(id)))
