...
async fn pump_subscription(
    ...
    // Send a typed ChannelFrame::Ready frame so clients (notably
    // the end_to_end test) can race-freely know the subscription
    // is installed and downstream publishes will be observed.
    let ready_frame = serde_json::to_string(&ChannelFrame::Ready)
        .expect("ChannelFrame::Ready serialises (unit variant, no payload)");
    ...
    loop {
        ...
        let frame = match serde_json::to_string(&event) {
            Ok(s) => s,
            Err(e) => {
                // Should be unreachable — every ChannelFrame
                ...
                tracing::error!(
                    error = %e,
                    "ChannelFrame serialise failed — emitting Lagged{{1}}"
                );
                let lagged =
                    serde_json::to_string(&ChannelFrame::Lagged { dropped: 1 })
                        .unwrap_or_else(|_| {
