...
pub fn is_docker_compose(value: &serde_yaml::Value) -> bool {
    if let serde_yaml::Value::Mapping(map) = value {
        // Docker Compose files typically have "services" and optionally "version"
        map.contains_key(&serde_yaml::Value::String("services".to_string()))
    } else {
        false
    }
}
...
pub fn find_matching_paths(value: &serde_yaml::Value, pattern: &[&str]) -> Vec<Vec<String>> {
...
pub fn get_value_at_path_mut<'a>(
    ...
    for segment in path {
        if let serde_yaml::Value::Mapping(map) = current {
            current = map.get_mut(&serde_yaml::Value::String(segment.clone()))?;
        } else {
            return None;
        }
    }
