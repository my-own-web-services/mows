...
pub fn workspace_docker_command(all: bool, path: Option<&Path>) -> Result<()> {
    ...
    if packages.is_empty() {
        return Err("No packages with docker-compose files found".to_string().into());
    }
...
fn find_nearest_docker_package(start: &Path, workspace_root: &Path) -> Result<PathBuf> {
    let mut current = start.to_path_buf();
...
fn find_docker_packages(workspace_root: &Path) -> Result<Vec<PathBuf>> {
...
#[derive(Debug, Deserialize)]
struct Workspace {
    #[allow(dead_code)] // Present in TOML but only used indirectly via has-field checks
...
struct WorkspaceDep {
    ...
    #[allow(dead_code)] // Parsed from TOML but not yet used in generation
    default_features: Option<bool>,
    #[allow(dead_code)] // Parsed from TOML but not yet used in generation
    optional: Option<bool>,
    #[allow(dead_code)] // Kept for potential future use in raw TOML output
    raw: toml::Value,
}
...
) -> Result<BTreeMap<String, PackageDepInfo>> {
...
fn generate_toml_content(
    ...
    output.push_str("# This file is generated by mows. Do not edit manually.\n");
...
fn validate_dependency_name(name: &str) -> Result<()> {
    if name.is_empty() {
        return Err("Dependency name cannot be empty".to_string().into());
    }
...
fn format_toml_section(prefix: &str, value: &toml::Value) -> Result<String> {
    let mut output = String::new();
...
mod tests {
    ...
    fn test_is_workspace_dep() {
        ...
        let string_dep = toml::Value::String("1.0".to_string().into());
        assert!(!is_workspace_dep(&string_dep));
    }

    #[test]
    fn test_parse_workspace_dep_string() {
        let value = toml::Value::String("1.2.3".to_string().into());
        let dep = parse_workspace_dep(&value);
        assert_eq!(dep.version, Some("1.2.3".to_string().into()));
    ...
    fn test_parse_workspace_dep_table() {
        ...
        let dep = parse_workspace_dep(&value);
        assert_eq!(dep.version, Some("1.0.0".to_string().into()));
    ...
    fn test_parse_workspace_dep_path() {
        ...
        let dep = parse_workspace_dep(&value);
        assert_eq!(dep.path, Some("./libs/mylib".to_string().into()));
