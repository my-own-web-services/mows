...
impl ClientBuilder<NoInterceptor> {
    ...
    pub fn with_service_account(
        ...
        let interceptor =
            ServiceAccountInterceptor::new(&self.api_endpoint, service_account, auth_options.clone());
...
async fn get_channel(api_endpoint: &str, channel_config: &ChannelConfig) -> anyhow::Result<Channel> {
...
mod tests {
    ...
    async fn clients_are_cloneable() {
        ...
        let service_account_client = ClientBuilder::new(ZITADEL_URL)
            .with_service_account(&ServiceAccount::load_from_json(SERVICE_ACCOUNT).unwrap(), None)
            .build_user_client()
            .await
            .unwrap();
